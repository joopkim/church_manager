...
- 터치 우선: 주요 사용자가 60대 이상일 수 있으므로 가상 키보드 입력을 최소화하고 큰 터치 버튼, 선택형 입력, 음성 메모를 우선한다.
...
- 모바일에서는 PC용 사이드바나 가로 메뉴를 그대로 노출하지 않는다.
- 모바일 내비게이션은 하단 고정 아이콘 버튼을 우선한다.
- 표는 모바일에서 카드형 행 또는 핵심 컬럼만 표시한다.
- 출석 체크는 모바일 최적화를 우선한다.
- 버튼과 입력창의 터치 영역은 최소 56px 이상을 권장한다.
- 긴 텍스트 입력은 기본 동선에서 피하고, 음성 녹음/음성 메모 버튼을 제공한다.

## Do

- 대시보드와 목록 중심의 운영 도구로 설계한다.
- 모바일 화면은 큰 글자, 큰 버튼, 짧은 문장, 선택형 입력을 우선한다.
...
- 모바일에서 PC용 사이드바나 작은 탭 메뉴를 그대로 재사용하지 않는다.
- 60대 이상 사용자가 자주 쓰는 흐름에 가상 키보드 입력을 강요하지 않는다.
